...
def test_resolve(mock_locate_answer, mock_parse_response, mock_query_servers,
    ...
    mock_randrange.assert_called_once_with(65536)
    mock_construct_query.assert_called_once_with(731, "www.whateva.org", resolver.DNSRecordType.A)
    mock_query_servers.assert_called_once_with(b'hello', ['7.7.7.7', '8.8.8.8'])
    mock_parse_response.assert_called_once_with(b'response', 731)
    mock_locate_answer.assert_called_once_with("www.whateva.org", False, "woohoo")
    ...
    mock_construct_query.assert_called_once_with(731, "www.whateva.org",
                                          resolver.DNSRecordType.MX)
    mock_locate_answer.assert_called_once_with("www.whateva.org", True, "woohoo")
